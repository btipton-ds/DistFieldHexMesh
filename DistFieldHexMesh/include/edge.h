...
public:
	...
	Edge(const Index3D& ownerBlockIdx, const UniversalIndex3D& vert0, const UniversalIndex3D& vert1);
...
private:
	...
	std::set<UniversalIndex3D> _faceIds; // Should be 2, but this allows creation of nonmanifold edges if needed.
};

inline Edge::Edge(const Index3D& ownerBlockIdx, const UniversalIndex3D& vert0, const UniversalIndex3D& vert1)
...
inline const std::set<UniversalIndex3D>& Edge::getFaceIds() const
{
	return _faceIds;
}
