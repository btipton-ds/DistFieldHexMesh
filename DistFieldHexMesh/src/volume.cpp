...
void Volume::createBlockRays(const TriMesh::CMeshPtr& pTriMesh, AxisIndex axisIdx, std::vector<bool>& blocksToCreate)
{
	...
	MultiCore::runLambda([this, &allHitsLock, pTriMesh, axisOrder, rayDir, &allHits, &blocksToCreate](size_t threadNum, size_t numThreads) {
		const size_t axis0 = axisOrder[0];
		const size_t axis1 = axisOrder[1];
		const size_t axis2 = axisOrder[2];
...
void Volume::createCellRays(const TriMesh::CMeshPtr& pTriMesh, AxisIndex axisIdx, const std::vector<bool>& blocksToCreate)
{
	mutex allHitsLock;
	const Vector3i axisOrder = getAxisOrder(axisIdx);
	...
	MultiCore::runLambda([this, &allHitsLock, pTriMesh, axisOrder, rayDir, &allHits](size_t threadNum, size_t numThreads) {
		const double TOL = 1.0e-6;
		const size_t blockDim = Block::getBlockDim();
		const size_t axis0 = axisOrder[0];
		const size_t axis1 = axisOrder[1];
		const size_t axis2 = axisOrder[2];
		const size_t numI = _blockDim[axis0] + 1;
		...
		for (int i = 0; i < 3; i++) {
			...
			blockSpan[i] = blockSpan[i] / blockDim;
		}

		for (size_t i = 0; i < numI; i++) {
			double t = i / (numI - 1.0);
			blockOrigin[axis0] = _originMeters[axis0] + t * _spanMeters[axis0];
			...
			for (size_t j = 0; j < numJ; j++) {
				...
				RayBlockIntersectVec& blockHits = iterY->second;
				if (blockHits.empty())
					continue;
				if (blockHits.size() == 1)
					blockHits.resize((blockDim + 1) * (blockDim + 1));

				double u = j / (numJ - 1.0);
				blockOrigin[axis1] = _originMeters[axis1] + u * _spanMeters[axis1];

				Vector3d cellOrigin(blockOrigin);
				for (size_t ii = 0; ii <= blockDim; ii++) {
					double tt = ii / (blockDim - 1.0);
					cellOrigin[axis0] = blockOrigin[axis0] + tt * blockSpan[axis0];

					for (size_t jj = 0; jj <= blockDim; jj++) {
						double uu = jj / (blockDim - 1.0);
						cellOrigin[axis1] = blockOrigin[axis1] + uu * blockSpan[axis1];
...
CMeshPtr Volume::buildCFDHexes(const CMeshPtr& pTriMesh, double minCellSize, const Vector3d& emptyVolRatio)
{
	...
	createCellRays(pTriMesh, AxisIndex::X, blocksToCreate);
	createCellRays(pTriMesh, AxisIndex::Y, blocksToCreate);
	createCellRays(pTriMesh, AxisIndex::Z, blocksToCreate);
