...
uniform sampler depthBlenderSampler;
...
vec4 ShadeFragment(
  ...
  vec4 color = intensity * fragColor;
...
void main(void)
{
	...
	vec2 depthBlender = textureRect(depthBlenderSampler, gl_FragCoord.xy).xy;
	vec4 forwardTemp = textureRect(frontBlenderSampler, gl_FragCoord.xy);
