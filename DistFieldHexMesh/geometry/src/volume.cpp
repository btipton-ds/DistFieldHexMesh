...
void Volume::divideSimple(const SplittingParams& params, ProgressReporter* pReporter, bool multiCore)
{
	for (; _splitNum < params.numSimpleDivs; _splitNum++) {
		runThreadPool_IJK([this](size_t threadNum, const BlockPtr& pBlk)->bool {
			pBlk->iteratePolyhedraInOrder([](const auto& cellId, Polyhedron& cell)->bool {
...
void Volume::doQualitySplits(const SplittingParams& params)
{
	if (_pAppData->getDoQualitySplits()) {
		...
		do {
			bool isTooComplex = false;
			runThreadPool([this, &params, &isTooComplex](size_t threadNum, const BlockPtr& pBlk) {
				pBlk->iteratePolyhedraInOrder([this, &params, &isTooComplex](const Index3DId& cellId, Polyhedron& cell)->bool {
			...
			if (isTooComplex) {
				bool changed = false;
				runThreadPool([this, &params, &changed](size_t threadNum, const BlockPtr& pBlk) {
					pBlk->iteratePolyhedraInOrder([this, &changed, &params](const Index3DId& cellId, Polyhedron& cell)->bool {
						cell.clearAxisSplitBits();
...
void Volume::finishSplits(const SplittingParams& params, bool doRequired)
{
...
void Volume::splitWithModel(const SplittingParams& params)
{
	if (!params.makeSplitFaces)
		return;

	runThreadPool_IJK([this, &params](size_t threadNum, const BlockPtr& pBlk)->bool {
		pBlk->iteratePolyhedraInOrder([this, &pBlk, &params](const Index3DId& cellId, Polyhedron& cell)->bool {
			size_t sl = -1;
			Splitter3D sp(pBlk.get(), cellId, sl);
			sp.splitWithModel();
...
void Volume::polymeshWrite(const string& dirPath, ProgressReporter* pReporter)
{
	...
	runThreadPool([this](size_t threadNum, const BlockPtr& pBlk) {
		pBlk->iterateVerticesInOrder([pBlk](const Index3DId& vertId, Vertex& vert)->bool {
...
bool Volume::verifyUniquePoints(bool multiCore) const
{
	for (size_t i = 0; i < _blocks.size(); i++) {
		...
		pBlk->iterateVerticesInOrder([this, i](const Index3DId& id0, const Vertex& vert)->bool {
