	...
	: wxDialog(parent, id, title, pos, wxSize(400, 460), wxDEFAULT_DIALOG_STYLE, wxString("Divide Mesh"))
{
	...
	_numGapDivsPrompt = new wxStaticText(this, 0, _T("# gap divs"), wxPoint(col0, baseRowPixels + rowNum * rowHeight), wxSize(promptWidth, boxHeight));
	_numGapDivsBox = new wxTextCtrl(this, NUM_GAP_DIVS_ID, std::to_string(params.numGapDivs), wxPoint(col1, baseRowPixels + rowNum * rowHeight - descent), wxSize(boxWidth, boxHeight), wxTE_RIGHT);
	rowNum++;
	...
	_minEdgeLengthBox = new wxTextCtrl(this, MIN_EDGE_LENGTH_ID, std::to_string(params.minEdgeLength), wxPoint(col1, baseRowPixels + rowNum * rowHeight - descent), wxSize(boxWidth, boxHeight), wxTE_RIGHT);
	rowNum++;
